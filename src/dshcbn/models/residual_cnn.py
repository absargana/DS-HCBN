...
class SharedMorphologyEncoder(nn.Module):
    """
    Shared local/context CNN encoder.
    Weight sharing deliberately constrains capacity and keeps the model close to the
    original dual-crop U-Net-style CNN branch while reducing scale-specific overfit.
    """
    def __init__(self, base: int = 16, out_dim: int = 256, drop: float = 0.05):
        ...
        self.s1 = ResBlock3D(base, base * 2, stride=2, drop=drop)
        self.s2 = ResBlock3D(base * 2, base * 4, stride=2, drop=drop)
        self.s3 = ResBlock3D(base * 4, base * 8, stride=2, drop=drop)
        self.s4 = ResBlock3D(base * 8, base * 12, stride=2, drop=drop)
        self.pool = nn.AdaptiveAvgPool3d(1)
        self.proj = nn.Sequential(
            nn.Linear(base * 12, out_dim),
...
class FocusedDSHCBN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.encoder = SharedMorphologyEncoder(cnn_base, fusion_dim, encoder_drop)
